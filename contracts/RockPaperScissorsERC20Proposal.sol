import "https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v3.3.0-rc.0/contracts/token/ERC20/ERC20.sol";
// SPDX-License-Identifier: MIT
pragma solidity >=0.4.21 <0.7.0;

contract RockPaperScissors is ERC20 {
    constructor() public ERC20("Game Coin", "GCOIN") {
        //Minting 1000 fungible tokens in the smart contract
        _mint(address(this), 1000);
    }

    // Using enum to have a fixed set of options
    enum Options {
        NOT_PLAYED,
        ROCK,
        PAPER,
        SCISSORS
    }

    // Used to show data about the finished game, it shows the choice of winner-loser and the address of the winner
    event Winner(
        string loserAnnouncement,
        string winnerAnnouncement,
        address winner
    );

    // Used to show when the players join the game
    event PlayerJoined(string announcement, address player);
    event NewBetPropposed(string announcement, address player);
    address private player1;
    address private player2;
    uint256 private currentBet = 0;
    bool private player1AcceptedBet = false;
    bool private player2AcceptedBet = false;
    Options private player1Choice = Options.NOT_PLAYED;
    Options private player2Choice = Options.NOT_PLAYED;
    // Deadline to return the fungible tokens
    uint256 public deadline;

    // It is needed to have at least to players to start the game
    modifier atLeastTwoPlayers() {
        require(
            player1 != address(0),
            "Please join the game before you choose an option"
        );
        require(
            player2 != address(0),
            "Can not choose an option before the player 2 joins the game"
        );
        _;
    }
    // Prevents players from changing their option during the game
    modifier alreadyPlayed() {
        if (msg.sender == player1 && player1Choice != Options.NOT_PLAYED) {
            revert("You already played");
        }
        if (msg.sender == player2 && player2Choice != Options.NOT_PLAYED) {
            revert("You already played");
        }
        _;
    }

    // Prevents players from joinning multiple times
    modifier alreadyJoined() {
        require(
            msg.sender != player1,
            "This address already joined as player 1"
        );
        require(
            msg.sender != player2,
            "This address already joined as player 2"
        );
        _;
    }

    // Modifier to allow only players to use some methods
    modifier isPlayer() {
        require(
            msg.sender == player1 || msg.sender == player2,
            "Only players can participate"
        );
        _;
    }

    // Function to allow the players to join the game, players will be assigned starting by player 1,
    // if it was assigned before then it assigns it to player2.
    function joinGame() external alreadyJoined() {
        // Prevents more than two players per game
        require(
            player1 == address(0) || player2 == address(0),
            "There is no space for new players"
        );
        // If the player1 has not been assigned before then assign the sender address to player1
        if (player1 == address(0)) {
            player1 = msg.sender;
            emit PlayerJoined("The player 1 joined the game", player1);
            // Transfering 500 initial fungible tokens to the player 1
            transfer(player1, 500);
        } else {
            // If the player2 has not been assigned before then assign the sender address to player2
            if (player2 == address(0)) {
                player2 = msg.sender;
                emit PlayerJoined("The player 2 joined the game", player2);
                // Transfering 500 initial fungible tokens to the player 2
                transfer(player2, 500);
            }
        }
    }

    // Returns the current bet amount
    function getCurrentBetAmount()
        external
        view
        isPlayer()
        returns (uint256 currentBetAmount)
    {
        return currentBet;
    }

    // Marks the player as they accepted the bet and also their bet tokens are retained by the smart contract
    function acceptBet() public isPlayer() atLeastTwoPlayers() {
        require(currentBet > 0, "There is no bet in progress");
        // If they have enough balance to do this transfer they can accept the bet
        // Lock the tokens in the smart contract so that players can not withdraw them to avoid paying
        transfer(address(this), currentBet);
        if (msg.sender == player1) {
            player1AcceptedBet = true;
        }
        if (msg.sender == player2) {
            player2AcceptedBet = true;
        }
    }

    // Ability to decline the bet, if this is called then the bet configuration gets restarted
    function declineBet() external isPlayer() atLeastTwoPlayers() {
        require(currentBet > 0, "There is no bet in progress");
        resetBet();
    }

    // Used to propose a bet amount
    function proposeBetAmount(uint256 proposedBet)
        external
        isPlayer()
        atLeastTwoPlayers()
    {
        require(
            currentBet == 0,
            "Unable to propose a bet because the is one in process"
        );
        // Deadline to retain the tokens is 15 min
        deadline = now + 600;
        currentBet = proposedBet;
        // The player who propose the bet has to accepted
        acceptBet();
    }

    // Used to reset the bet configuration
    function resetBet() internal {
        player1AcceptedBet = false;
        player2AcceptedBet = false;
        currentBet = 0;
        deadline = 0;
    }

    // Function to pay the prize of the bet to the winner
    // if there is not winner then it returns the bet tokens
    function payTheWinner(address winner) internal {
        if (currentBet > 0) {
            // Sending the prize to winner
            if (player1AcceptedBet == true && player2AcceptedBet == true) {
                transferFrom(address(this), winner, currentBet);
            }
            // Sending the bet tokens back to player1
            if (player1AcceptedBet == true && player2AcceptedBet == false) {
                transferFrom(address(this), player1, currentBet);
            }
            // Sending the bet tokens back to player2
            if (player1AcceptedBet == true && player2AcceptedBet == false) {
                transferFrom(address(this), player1, currentBet);
            }
            // Reset bet configuration after game
            resetBet();
        }
    }

    // Returns the current bet amount to the players in case the opponent does not accept the bet
    function verifyBet() internal {
        if (currentBet > 0) {
            if (player1AcceptedBet == true && player2AcceptedBet == false) {
                transferFrom(address(this), player1, currentBet);
            }
            if (player2AcceptedBet == true && player1AcceptedBet == false) {
                transferFrom(address(this), player2, currentBet);
            }
            resetBet();
        }
    }

    // This can be used to return the bet tokens if the opponent did not want to play anymore
    function getBalanceOf(address account) public returns (uint256) {
        if (deadline < now) {
            verifyBet();
        }
        return super.balanceOf(account);
    }

    // Using external since it is cheaper than public and nobody needs to call it internally
    // Function to select the players option and the game, and if both players are selected an option then choose a winner
    function chooseAnOption(Options _playerChoice)
        external
        atLeastTwoPlayers()
        alreadyPlayed()
        isPlayer()
    {
        // Disables this NOT_PLAYED option from the game
        require(
            _playerChoice != Options.NOT_PLAYED,
            "The choice NOT PLAYED is not valid"
        );
        if (msg.sender == player1) {
            player1Choice = _playerChoice;
        }
        if (msg.sender == player2) {
            player2Choice = _playerChoice;
        }
        if (
            player1Choice != Options.NOT_PLAYED &&
            player2Choice != Options.NOT_PLAYED
        ) {
            selectWinner();
        }
    }

    // The rock defeats the scissors by breaking it, the scissors that defeats the paper by cutting it
    // and the paper defeats the stone by wrapping it, if the option is the same then it is a draw
    function selectWinner() internal {
        Options winnerChoice;
        Options loserChoice;
        address winner;
        // It is a draw
        if (player1Choice == player2Choice) {
            winnerChoice = player1Choice;
            loserChoice = winnerChoice;
            winner = address(0);
        }
        // The winner is player1
        else if (
            (player1Choice == Options.ROCK &&
                player2Choice == Options.SCISSORS) ||
            (player1Choice == Options.SCISSORS &&
                player2Choice == Options.PAPER)
        ) {
            winnerChoice = player1Choice;
            loserChoice = player2Choice;
            winner = player1;
        }
        // The winner is player2
        else {
            winnerChoice = player2Choice;
            loserChoice = player1Choice;
            winner = player2;
        }
        // Announce the loser and winner
        string memory loserAnnouncement = getAnnouncement(
            loserChoice,
            "The loser chose: "
        );
        string memory winnerAnnouncement = getAnnouncement(
            winnerChoice,
            "The winner chose: "
        );
        emit Winner(loserAnnouncement, winnerAnnouncement, winner);
        payTheWinner(winner);
        // After the winner was announced, the game is reset
        reset();
    }

    // Creates an announcement message for the players
    function getAnnouncement(Options option, string memory message)
        internal
        pure
        returns (string memory announcement)
    {
        string memory choice = getMyOptionKeyByValue(option);
        return string(abi.encodePacked(message, choice));
    }

    // Convert the enum to string
    function getMyOptionKeyByValue(Options option)
        internal
        pure
        returns (string memory selectedOption)
    {
        if (Options.ROCK == option) return "Rock";
        if (Options.SCISSORS == option) return "Scissors";
        if (Options.PAPER == option) return "Paper";
        if (Options.NOT_PLAYED == option) return "Not played";
    }

    function reset() internal {
        player1Choice = Options.NOT_PLAYED;
        player2Choice = Options.NOT_PLAYED;
    }

    // Using getter because address modification is not allowed
    function getPlayers() external view returns (address, address) {
        return (player1, player2);
    }
}
